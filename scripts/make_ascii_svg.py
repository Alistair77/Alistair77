...
CONTRAST = 1.30      # higher contrast -> crisper highlights/shadows on the face
BRIGHTNESS = 1.0
GAMMA = 1.02         # near-linear mids so the face keeps its shading, not a blob
SHARPEN = False
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">Alistair77@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'Alistair77@github:~$ whoami <tspan fill="{INK}">Alistair</tspan></text>')
